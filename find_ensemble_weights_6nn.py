...
pca = PCA()#(n_components=(X.shape[1]-20), whiten=True) #()
...
for b in range(nbag):
    ...
    for i, (train_index, test_index) in enumerate(folds):
        ...
        hist = model.fit(X_train, y_train, nb_epoch= 12, batch_size=50, validation_data=(X_val, y_val)) #, checkpoint , callbacks=[early]
...
data[["id", "loss"]].to_csv(loc+"submissions/submission_avg6nn"+str(final_score)+".csv", index=False)
