...
if __name__ == "__main__":
    ...
    ids_t = pd.read_csv(directory + 'train.csv')['id']
    ...
    ## train predictions
    df = pd.DataFrame({'id': ids_t, 'loss': pred_oob})
    df.to_csv(loc+'stacking_preds/preds_oob_xgb_'+'_5fold-average-xgb_fairobj_'+'.csv', index = False)

    score = str(round((cv_sum / n_folds), 6))
    sub_file = loc+'stacking_preds/submission_5fold-average-xgb_fairobj_' + str(score)  + '.csv'
