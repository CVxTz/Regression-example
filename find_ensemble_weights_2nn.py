...
def create_model2(inputsize):
    def model_fn():
        model = Sequential()
        model.add(Dense(500, input_dim = inputsize, init = 'he_normal'))
...
for b in range(nbag):
    ...
    for i, (train_index, test_index) in enumerate(folds):
        ...
        hist = model.fit(X_train, y_train, nb_epoch= 1000, batch_size=50, validation_data=(X_val, y_val), callbacks=[early, checkpoint])
...
data[["id", "loss"]].to_csv(loc+"submissions/submission_avg"+str(final_score)+".csv", index=False)
